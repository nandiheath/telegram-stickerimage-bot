...
function finishHandler(ctx, imopts) {
  ...
  async.series([
    ...
    function (cb) {
      ...
      uploadHandler(ctx, fpath, function (err, zip) {
        cb(err, zip);
      });
    }],
    function (err, res) {
...
function uploadImage(packId, imagePath, callback) {
  request.post({
    url: `${HOSTNAME}/pack/${packId}/image`,
    headers: {
...
function uploadTray(packId, imagePath, callback) {
  request.post({
    url: `${HOSTNAME}/pack/${packId}/tray_icon`,
    headers: {
      Authorization: `Bearer ${API_TOKEN}`
    },
    formData: {
...
function publishPack(packId, callback) {
  request.post({
    url: `${HOSTNAME}/pack/${packId}`,
    headers: {
      Authorization: `Bearer ${API_TOKEN}`
    },
    json: {
...
function uploadHandler(ctx, fpath, callback) {
  let chatId = ctx.message.chat.id;
  const setName = ramdb[chatId].setName;
  logger(chatId, 'info', `Uploading ${setName}...`);

  if (ramdb[chatId].srcimg.length > 30) {

    
    const tasks = [];
    const numOfPacks = Math.ceil(ramdb[chatId].srcimg.length / 30.0);
    logger(chatId, 'info', `We will split the sticker set to ${numOfPacks} packs`);
    for (let i = 0; i < numOfPacks; i ++) {
      const startIndex = i * 30;
      const size = startIndex + 30 > ramdb[chatId].srcimg.length ? ramdb[chatId].srcimg.length - startIndex : 30;
      if (size > 3) {
        tasks.push({
          name: `${setName} #${i + 1}`,
          files: ramdb[chatId].srcimg.slice(startIndex, startIndex + size)
        })
      }
    }
    ...
    async.eachSeries(tasks, function ({name, files}, cb) {
      uploadPack(name, files, cb);
    }, function (err) {
      logger(chatId, 'info', `All file uploaded.`);      
      callback(err);
    });

  } else {
    uploadPack(setName, ramdb[chatId].srcimg, callback);
  }
  
}

function uploadPack(name, filePaths, callback) {
  request.post({
    url: `${HOSTNAME}/pack`,
    headers: {
      Authorization: `Bearer ${API_TOKEN}`
    ...
    const packId = body.data._id;

    uploadTray(packId, filePaths[0], (err) => {
      if (err) {
        return callback(err);
      }
      async.eachSeries(filePaths, function (src, cb) {
        uploadImage(packId, src, cb);
      }, function (err) {
        publishPack(packId, callback);        
      });
    })
  })
}
